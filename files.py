...
def handle_tcxFile(f):
    ...
    fd = {}
    dom = xmldom.parse(f)
    fd['activity'] = dom.getElementsByTagName('Activity')[0] \
        .getAttribute('Sport')
    fd['startDate'] = dom.getElementsByTagName('Lap')[0]\
        .getAttribute('StartTime')[:10]
    fd['allTrackPoints'] = dom.getElementsByTagName('Trackpoint')
    fd['startT'] = time.mktime(
        time.strptime(getValue('Time', fd['allTrackPoints'][0]), timeFormat))
...
def readFile(fname):
    names = ['Time', 'LatitudeDegrees', 'LongitudeDegrees',
             'AltitudeMeters', 'DistanceMeters', 'Value',
             'Cadence', 'SensorState']
    convertF = [lambda x: datetime.strptime(x, timeFormat),
